...
import web_cardapio.com.embarcadero.javaandroid.DSAdmin.NotifyCallbackReturns;
...
public class DSCallbackChannelManager {
	...
	private static final Logger log = Logger.getLogger(DSCallbackChannelManager.class);
	...
	static class WorkerThread extends Thread {
		...
		private TJSONObject channelCallbackExecute() throws Exception {
			...
			while (!stopped) {
				try {
					log.info(DSRESTConnection.TAG + " Listening for channel " + mngr.getChannelName() + " (retries " + String.valueOf(retries) + ")");
					...
					log.info(DSRESTConnection.TAG + " Got a message for " + mngr.getChannelName() + " (retries " + String.valueOf(retries) + ")");
					break;
				} catch (DBXException e) {						
						...
						{
							log.info(DSRESTConnection.TAG + " Timeout for channel " + mngr.getChannelName() + " (retries " + String.valueOf(retries) + ")");
							...
							{
								log.info(DSRESTConnection.TAG + " Raising exception after " + String.valueOf(retries) + " retries for " + mngr.getChannelName());
							...
							retries++;							
							try {
								log.info(DSRESTConnection.TAG + " Sleeping between attempt (retries " + String.valueOf(retries) + ") for " + mngr.getChannelName());
								Thread.sleep(this.mngr.getRetryDelay()); } catch (InterruptedException ie) {}
						}							
						else
						{
							log.info(DSRESTConnection.TAG + " Raising exception for channel " + mngr.getChannelName() + " (retries " + String.valueOf(retries) + ")");
